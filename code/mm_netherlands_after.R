...
mm_nl <- prepare_the_regional_data("nl", 1000, 1200)
...
ggplot(data.frame(mm_nl$bootresultd[[2]])) +
  aes(0.25, 
      slope) +
  geom_boxplot() +
  ...
  annotate("segment",
           y =      mm_nl$slope_max,
           yend =   mm_nl$slope_max,
           x = 0,
           xend = 0.5,
           colour = "red") +
  annotate("segment",
           y =     mm_nl$slope_min,
           yend =  mm_nl$slope_min,
           x =    0,
           xend = 0.5,
           colour = "red") +
  annotate("text",
  ...
  scale_y_continuous(limits = c(-1.75, 0.0))

ggsave("mm_netherlands_after.pdf")
