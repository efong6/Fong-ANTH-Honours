...
bd_plus_100y <- 
  x %>% 
  filter(between(decade, 1140, 1340))

bd_plus_100y_prepped <- roev::DataPrep(bd_plus_100y)
...
# slope parameters 
bootresultd = roev::TriPanelBC(bd_plus_100y_prepped,     # idrx matrix
                               "r",       # mode (diff/rate)
...
ggsave("Black-Death_Overall_200-Before.pdf")
